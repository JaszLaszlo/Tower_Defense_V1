...
public:
	Enemy(Vec2<float> p, float h, float sp, float si, const Map& map);
	virtual ~Enemy();
...
{
public:
	FastEnemy(Vec2<float> p, const Map& map) : Enemy(p, 50.0,85.0,55.0, map) {}
...
{
public:
	NormalEnemy(Vec2<float> p, const Map& map) : Enemy(p, 100.0, 55.0,30.0, map) {}
	void draw(Graphics& g) const;
...
{
public:
	TankEnemy(Vec2<float> p, const Map& map) : Enemy(p, 200.0, 40.0,30.0, map) {}
	void draw(Graphics& g) const;
